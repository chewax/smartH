(function(){
    ...
    function registerBoard ( board, socket ) {
        ...
        if (!isListed(boards, board.id)) {
            ...
            log.debug("enable");
            enable(boards, board, socket, 'board:enabled');
            socket.to('consoles').emit('console:board:enable', board);
        }
    ...
    function registerConsole (client, socket) {
        ...
        if (!isListed(consoles, client.id)) {
            ...
            enable(consoles, client, socket, 'console:enable')
    ...
    function enable (elements, e, socket, event) {
        //search for socket by boardId, change status, and reassigne socket
        let index = _.findIndex(elements, (e) => {
            return e.id.toString() == e.id.toString();
        })
