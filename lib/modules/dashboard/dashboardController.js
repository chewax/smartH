...
function boardBroadcast (payload) {
    _io.to('boards').emit(payload.message);
}
...
const renderDashboard = function (req, res) {
    ...
    res.render('dashboard', {
        devices: boards,
        scenes: [... new Set(boards.map(b => b.scene))]
    });
};
...
const initIO = function (socket, io) {
    ...
    socket.on('board:broadcast', payload => boardBroadcast(payload));
};
