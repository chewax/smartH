    ...
    class BoardManager {
        ...
        register (board, socket) {
            ...
            if (idx == -1) {
                ...
                boards[idx].enable(socket);
                this.io.to('consoles').emit('console:board:enable', board.strip());
            }
        ...
        processBoardRequest (data) {
            //TODO
            log.log(`Board request: ${data}`);
        ...
        getBoardBySocketId (socketId) {
            ...
            if (idx != -1) return boards[idx];
            else return {}
        }

        disableBoardBySocketId (socketId) {
            let board = this.getBoardBySocketId()
            
            if (!_.isEmpty(board)) {
                this.io.to('consoles').emit('console:board:disable', board.id);
            }
